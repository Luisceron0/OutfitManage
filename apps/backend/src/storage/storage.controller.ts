...
import { StorageService, UploadResult, UploadedMulterFile } from './storage.service';
...
export class StorageController {
  ...
  @Roles('ADMIN', 'BODEGA')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Subir una imagen o video a Supabase Storage y obtener su URL firmada' })
  ...
  @Roles('ADMIN', 'BODEGA')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Subir múltiples imágenes o videos simultáneamente' })
  @ApiConsumes('multipart/form-data')
  @UseInterceptors(
  ...
  async uploadMultiple(
    ...
    if (!files || files.length === 0) {
      throw new BadRequestException('Debes enviar al menos un archivo en el campo "files"');
    }
